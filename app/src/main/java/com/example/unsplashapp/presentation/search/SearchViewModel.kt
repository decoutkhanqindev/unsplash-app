...
class SearchViewModel(private val repository: UnsplashRepository) : ViewModel() {
  ...
  private suspend inline fun <reified T> searchItems(query: String): List<T> = try {
    val responseItems: List<T> = when (T::class) {
      PhotoItemModel::class ->
        repository.searchPhotos(query, 1, 30).results.map { it.toPhotoItemModel() } as List<T>
      
      UserItemModel::class ->
        repository.searchUsers(query, 1, 30).results.map { it.toUserItemModel() } as List<T>
